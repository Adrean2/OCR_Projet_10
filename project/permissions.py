...
class IsContributor(BasePermission):
    ...
    def has_permission(self, request, view,):
        if not view.kwargs:
            ...
            contributor = models.Contributor.objects.filter(project__id=view.kwargs["project_pk"]).filter(user__id=request.user.id)
        else:
            contributor = models.Contributor.objects.filter(project__id=view.kwargs["pk"]).filter(user__id=request.user.id)
